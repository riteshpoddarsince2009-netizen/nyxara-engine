# =========================================================================
# NYXARA OS V8 — THE ULTIMATE ELECTRIC REFRESH STUDIO
# FRONT-FACING CONTROL MATRIX + ANTI-OVERLAP TEXT ENGINE
# =========================================================================
...
# =========================================================
st.set_page_config(
    page_title="NYXARA OS V8 Electric Refresh",
    page_icon="⚡",
    layout="wide",
)
...
st.markdown("""
    ...
    div[data-testid="stForm"], .stTextArea, .stTextInput, .stNumberInput {
        ...
        box-shadow: 0 4px 20px rgba(0, 82, 212, 0.02) !important;
    }
    .section-container {
        background: #F8FAFC;
    ...
    .stButton>button:hover {
        ...
        box-shadow: 0 12px 35px rgba(67, 100, 247, 0.5) !important;
    }
    </style>
""", unsafe_allow_html=True)

# XML Sanitizer for Safe Rendering
...
# =========================================================
# PDF DESIGN SCHEMATIC (ELECTRIC APPLE STYLE)
# =========================================================
...
# Strict Metrics: Synchronized Font Sizes and Line Leadings to prevent overlapping
...
class ElectricStudioCanvas(canvas.Canvas):
    ...
    def draw_decorations(self):
        ...
        self.drawRightString(PAGE_WIDTH - 54, PAGE_HEIGHT - 37, "PRODUCTION MODULE DEPLOYED")
        ...
        self.drawString(54, 32, "Creative Operations Core Architecture File")
...
def render_prompt_card(story, item, current_display_num, is_bonus=False):
    ...
    # Clean Padded Box Wrapper around the Raw Prompt
    if item.get("prompt"):
...
# --- VISIBLE MAIN PAGE CONTROLS (AS REQUESTED) ---
st.markdown("### 🎛️ SYSTEM ARCHITECTURE SETTINGS")
col_inputs_1, col_inputs_2 = st.columns(2)

with col_inputs_1:
    st.markdown("<div class='section-container'>", unsafe_allow_html=True)
    ui_main_count = st.number_input("👉 Number of Main Prompts", min_value=1, value=4, help="Core system components count")
    st.markdown("</div>", unsafe_allow_html=True)

with col_inputs_2:
    st.markdown("<div class='section-container'>", unsafe_allow_html=True)
    ui_bonus_count = st.number_input("👉 Number of Bonus Prompts", min_value=0, value=1, help="Bonus follow-through modules count")
    st.markdown("</div>", unsafe_allow_html=True)
...
if st.button("🚀 COMPILE ARCHITECTURE & GENERATE PDF", use_container_width=True):
    ...
    try:
        ...
        st.error(f"Payload Compilation Error: Python list structural parser failure. Details: {failure_reason}")
        
    if sanitized_array is not None:
        if not isinstance(sanitized_array, list):
            st.error("Structural Mismatch Error: Input data must wrap inside a standard Python list format array.")
        else:
            actual_count = len(sanitized_array)
            expected_count = ui_main_count + ui_bonus_count
            
            if actual_count != expected_count:
                st.warning(f"💡 Auto-Adjusting Counts: Aapke array mein {actual_count} items hain, par upar configuration mein Total {expected_count} set tha. Humne automatically array ke hisab se numbers manage kar diye hain!")
                # Dynamic safe auto-fallback so it NEVER crashes
            ...
            # File Operations Matrix Mapping
            ...
            st.success("Compilation complete. Zero clashing verified. Prompt boxed beautifully!")
