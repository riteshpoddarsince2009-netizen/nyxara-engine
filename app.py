# =========================================================
# NYXARA OS V13 — FINAL STABLE ENGINE
# FIXED:
...
COLORS = {
    ...
    "paper": "#FFFFFF",
    "text": "#0F172A",
    "muted": "#334155",   # darker for visibility
...
def sanitize_filename(name):
    ...
    return name.strip("_") or "nyxara_os_v13"
...
def parse_data(raw):
    ...
    Supports:
    - one list literal
    - multiple `data = [...]` blocks pasted together
    - unlimited prompts
    """
    raw = clean_text(raw)
    ...
    # 2) Try parsing the whole script and collecting all assignments to `data`
    collected = []
    try:
        tree = ast.parse(raw)
...
def render_system(story, item, number, bonus=False):
    ...
    # Keep heading and prompt together if there is not enough space on the page.
...
st.markdown(
    f"""
    <div class="title">NYXARA OS V13</div>
...
raw = st.text_area(
    "Paste Prompt Data",
    height=420,
    help="You can paste one data block or multiple `data = [...]` blocks. The engine will compile them all."
)
