...
class GlassPromptCard(Flowable):
    ...
    def split(self, availWidth, availHeight):
        ...
        first = " ".join(words[:best]).strip()
        second = " ".join(words[best:]).strip()
...
# =========================================================
# PAGE BACKGROUNDS / HEADER / FOOTER
# =========================================================

def draw_cover_page(canvas_obj, doc):
    """
    Cover page is kept clean unless there is no cover image.
    This function does not draw header/footer over user cover art.
    """
...
def generate_pdf(data, bundle_title, main_count, bonus_count, cover_path=None, logo_path=None):
    ...
    # Create cover template and content template
    doc = BaseDocTemplate(
        out_path,
        pagesize=letter,
        leftMargin=55,
    ...
    full_frame = Frame(0, 0, PAGE_WIDTH, PAGE_HEIGHT, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0, showBoundary=0)
    content_frame = Frame(
        55, 55,
        PAGE_WIDTH - 110,
        PAGE_HEIGHT - 110,
    ...
    # let callbacks know about cover/logo
