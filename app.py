...
# ✅ CRITICAL FIX: Bracket-Closure & Multi-Block Extension Crash Resolved
# ✅ HOTFIX: Fixed spacer() unexpected keyword argument 'shadow_box' crash
# =========================================================
...
def parse_data(raw):
    """
    UNBREAKABLE MULTI-BLOCK PARSER:
    Completely isolates individual dictionaries. Outer closing brackets, 
    extra variable names, or text extensions will NEVER cause a crash.
    """
    raw = clean_text(raw)
    ...
    # Scans explicitly for inner dict boundaries, completely ignoring external bracket state resets
    while i < length:
        if raw[i] == '{':
            start_idx = i
            brace_count = 0
...
def render_system(story, item, number, bonus=False):
    ...
    for heading, content in sections:
        ...
        spacer(story, 14)  # STABLE ARGS RESTORED
