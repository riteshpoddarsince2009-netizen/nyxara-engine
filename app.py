...
# ✅ CRITICAL FIX: Bulletproof item parsing (No 5-prompt truncation)
# =========================================================
...
def parse_data(raw):
    """
    REPAIRED CORE PARSER: Extracts 100% of the prompts element-by-element 
    even if individual text blocks have syntax anomalies or quote leaks.
    """
    raw = clean_text(raw)
    raw = raw.replace("```python", "").replace("```", "").strip()
    
    # Normalize potential smart quotes from copy-paste
    raw = raw.replace("“", '"').replace("”", '"').replace("‘", "'").replace("’", "'")

    # 1) Try a single list literal first
    try:
    ...
    # 2) Deep AST traversal with element-level fallback protection
    try:
        tree = ast.parse(raw)
        for node in tree.body:
            if isinstance(node, ast.Assign):
                if any(isinstance(t, ast.Name) and t.id == "data" for t in node.targets):
                    try:
                        # Try parsing the complete block assignment
                        value = ast.literal_eval(node.value)
                        if isinstance(value, list):
                            collected.extend(value)
                    except:
                        # CRITICAL FIX: If full block eval fails, process each item one by one
                        if isinstance(node.value, ast.List):
                            for elt in node.value.elts:
                                try:
                                    item = ast.literal_eval(elt)
    ...
    # 3) Absolute Last-Resort Strategy: Regex Extraction for flat structural patterns
    try:
        dict_blocks = re.findall(r"\{\s*[\"']title[\"'].*?\}\s*(?=\,|\s*\]|\s*\}|\s*\{|\Z)", raw, re.DOTALL)
        for block in dict_blocks:
            try:
                cleaned_block = re.sub(r",\s*\}", "}", block.strip())
                item = ast.literal_eval(cleaned_block)
                if isinstance(item, dict) and "title" in item and item not in collected:
                    collected.append(item)
            except:
                pass
    except:
