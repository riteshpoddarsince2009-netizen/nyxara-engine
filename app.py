...
# ✅ Bracket-Closure & Multi-Block Extension Crash Resolved
# ✅ CRITICAL UPDATE: Ultra-Premium Dynamic Front Page Layout (Centered Logo & Footer)
# =========================================================
...
COVER_COPYRIGHT = ParagraphStyle(
    "COVER_COPYRIGHT",
    parent=styles["Normal"],
    fontName="Helvetica",
    fontSize=9,
...
def draw_cover_page(canvas_obj, doc):
    ...
    cover = getattr(doc, "_cover_image", None)

    if cover and os.path.exists(cover):
        try:
            canvas_obj.drawImage(cover, 0, 0, width=PAGE_WIDTH, height=PAGE_HEIGHT)
        except:
            pass
    else:
...
def draw_page(canvas_obj, doc):
    ...
    if logo and os.path.exists(logo):
        try:
            canvas_obj.drawImage(logo, 55, PAGE_HEIGHT - 42, width=16, height=16, mask="auto")
...
def render_cover_story(story, bundle_title, total, main_count, bonus_count, logo_path=None):
    """
    Renders an elite, ultra-premium, perfectly centered front page workflow story.
    Fully dynamic prompt count readout and structural layout.
    """
    story.append(Spacer(1, 100))
    
    # 1. Centered Big Corporate Logo
    if logo_path and os.path.exists(logo_path):
        try:
            story.append(Image(logo_path, width=85, height=85, hAlign='CENTER'))
            story.append(Spacer(1, 25))
        except:
            pass

    # 2. Premium Branding Tag
    story.append(Paragraph("— NYXARA PRIVÉ // SPECIFICATION —", COVER_TAG))
    story.append(Spacer(1, 15))

    # 3. Core Title
    story.append(Paragraph(bundle_title.upper(), COVER_HERO))
    story.append(Spacer(1, 22))
    
    # 4. Dynamic Working Prompt Counter Metrics
    stats_string = f"[CORE: {main_count} SYSTEMS]   •   [BONUS: {bonus_count}]   •   [TOTAL: {total} UNITS]"
    story.append(Paragraph(stats_string, COVER_SUB))
    story.append(Spacer(1, 25))
    ...
    # 6. Smooth Push Spacer to position the Copyright notice at the absolute bottom cleanly
    story.append(Spacer(1, 130))
    
    # 7. Copyright Notice
    story.append(Paragraph("© 2026 All Rights Reserved", COVER_COPYRIGHT))
...
def generate_pdf(data, bundle_title, main_count, bonus_count, cover=None, logo=None):
    ...
    doc._logo = logo
    doc._cover_image = cover
    doc._has_cover_image = bool(cover and os.path.exists(cover))

    cover_frame = Frame(
        55, 55, PAGE_WIDTH - 110, PAGE_HEIGHT - 110,
    ...
    # =====================================================
    # OVERLAY DYNAMIC FRONT PAGE OVER CANVAS BACKGROUND
    # =====================================================
    render_cover_story(
        story,
        bundle_title,
